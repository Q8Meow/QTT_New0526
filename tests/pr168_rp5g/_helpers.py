...
ART_DIR = REPO_ROOT / "docs" / "master_plan" / "generated" / "pr168_rp5g"
...
def assert_valid() -> dict:
    from src.qtt.stage1_prediction_markets.pr168_rp5g_trade_plan_sim.runner import run_layer
    from src.qtt.stage1_prediction_markets.pr168_rp5g_trade_plan_sim.validator import run_validation

    run_layer(offline=True, fixture="sample", max_candidates=10, timeout_ms=3600000)
    result = run_validation()
    assert result["validation"] == "PR168_RP5G_TRADE_PLAN_SIM_OK"
    return result
...
def assert_common_contract(row: dict) -> None:
    ...
    assert row["source_pr"] == "PR168-RP5G"
    assert row["upstream_refs"]
    assert row["downstream_refs"]
    assert row["consumer_agents"]
    assert row["validation_refs"]
    assert row["execution_authority_ref"].startswith("RP5G_EXEC_AUTH::")
    assert row["blocker_policy_ref"].startswith("RP5G_BLOCKER_POLICY::")
