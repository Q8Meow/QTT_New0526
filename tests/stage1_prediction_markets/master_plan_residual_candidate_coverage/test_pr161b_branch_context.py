...
def test_pr161b_pr162r_b_downstream_branch_allows_cumulative_validation():
    assert (
        validator._branch_context_allowed(
            validator.Path(__file__).resolve().parents[3],
            PR162R_B_DOWNSTREAM_BRANCH,
...
def test_pr161b_pr163_downstream_branch_allows_cumulative_validation():
    assert (
        validator._branch_context_allowed(
            validator.Path(__file__).resolve().parents[3],
            PR163_DOWNSTREAM_BRANCH,
...
def test_pr161b_detached_head_pr163_c_repair_pr_context_allowed(monkeypatch):
    ...
    assert failures == ()
    assert receipts == ("PR161B_REASON_CI_DETACHED_HEAD_RELAXATION_BRANCH_ONLY",)


def test_pr161b_detached_head_unrelated_repair_head_ref_remains_blocked(
    monkeypatch,
):
    failures, receipts = _detached_branch_outcome(
        monkeypatch,
    ...
    assert failures == ("PR161B_BLOCKED_WRONG_BRANCH:DETACHED_HEAD",)
    assert receipts == ()
