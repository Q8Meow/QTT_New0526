...
from src.qtt.stage1_prediction_markets.pr166_qb_bounded_quantum_benchmark.authority import (
    FORBIDDEN_AUTHORITY_FLAGS,
    ZERO_AUTHORITY_KEYS,
)
from src.qtt.stage1_prediction_markets.pr166_qb_bounded_quantum_benchmark.io import (
    read_json,
...
def summary() -> dict:
    return records("PR166_QB_FinalSummary.report.json")[0]
...
def assert_report_contract(filename: str, expected: int | None = None) -> tuple[dict, ...]:
    ...
    assert payload(filename)["roadmap_pr_id"] == "PR166-QB"
    assert payload(filename)["created_by_pr"] == "PR166-QB"
