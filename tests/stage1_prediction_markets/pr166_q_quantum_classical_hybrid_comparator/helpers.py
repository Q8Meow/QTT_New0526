...
from src.qtt.stage1_prediction_markets.pr166_q_quantum_classical_hybrid_comparator.authority import FORBIDDEN_AUTHORITY_FLAGS, ZERO_AUTHORITY_KEYS
from src.qtt.stage1_prediction_markets.pr166_q_quantum_classical_hybrid_comparator.io import read_json, records_from_report_payload
...
def summary() -> dict:
    return records("PR166_Q_FinalSummary.report.json")[0]
...
def assert_report_contract(filename: str, expected: int | None = None) -> tuple[dict, ...]:
    ...
    assert payload(filename)["roadmap_pr_id"] == "PR166-Q"
    assert payload(filename)["created_by_pr"] == "PR166-Q"
