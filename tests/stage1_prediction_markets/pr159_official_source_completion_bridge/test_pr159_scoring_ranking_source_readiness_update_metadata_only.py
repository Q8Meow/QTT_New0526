...
def test_pr159_scoring_ranking_source_readiness_update_metadata_only():
    payload = report(c.SCORING_RANKING_SOURCE_UPDATE_PATH)
    assert payload["record_count"] == 845
    assert all(item["metadata_only_no_scoring_execution"] is True for item in payload["records"])
