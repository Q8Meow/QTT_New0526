...
from src.qtt.stage1_prediction_markets.pr162e_q_quantum_automapper.authority import (
...
def summary() -> dict:
    return records("PR162E_Q_FinalSummary.report.json")[0]
...
def assert_report_contract(filename: str, expected: int | None = None) -> tuple[dict, ...]:
    ...
    report = payload(filename)
    assert report["roadmap_pr_id"] == "PR162E-Q"
    assert report["created_by_pr"] == "PR162E-Q"
