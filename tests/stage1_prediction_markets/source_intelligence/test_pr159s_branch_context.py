...
def test_pr159s_pr162r_a_downstream_branch_allows_cumulative_validation(monkeypatch):
    monkeypatch.delenv("GITHUB_ACTIONS", raising=False)
    monkeypatch.delenv("GITHUB_EVENT_NAME", raising=False)
    monkeypatch.delenv("GITHUB_REF", raising=False)
    monkeypatch.delenv("GITHUB_REF_NAME", raising=False)
    ...
    def fake_git_stdout(repo_root, args):
        command = tuple(args)
        if command == ("branch", "--show-current"):
            return 0, PR162R_A_DOWNSTREAM_BRANCH, ""
        if command == ("rev-parse", "--abbrev-ref", "HEAD"):
            return 0, PR162R_A_DOWNSTREAM_BRANCH, ""
        if command[:2] == ("merge-base", "--is-ancestor"):
            return 1, "", "not ancestor"
...
def test_pr159s_pr162d_r2a_downstream_branch_allows_cumulative_validation(monkeypatch):
    monkeypatch.delenv("GITHUB_ACTIONS", raising=False)
    monkeypatch.delenv("GITHUB_EVENT_NAME", raising=False)
    monkeypatch.delenv("GITHUB_REF", raising=False)
    monkeypatch.delenv("GITHUB_REF_NAME", raising=False)
...
def test_pr159s_pr162r_b_downstream_branch_allows_cumulative_validation(monkeypatch):
    monkeypatch.delenv("GITHUB_ACTIONS", raising=False)
    monkeypatch.delenv("GITHUB_EVENT_NAME", raising=False)
    monkeypatch.delenv("GITHUB_REF", raising=False)
    monkeypatch.delenv("GITHUB_REF_NAME", raising=False)
    ...
    def fake_git_stdout(repo_root, args):
        command = tuple(args)
        if command == ("branch", "--show-current"):
            return 0, PR162R_B_DOWNSTREAM_BRANCH, ""
        if command == ("rev-parse", "--abbrev-ref", "HEAD"):
            return 0, PR162R_B_DOWNSTREAM_BRANCH, ""
        if command[:2] == ("merge-base", "--is-ancestor"):
            return 1, "", "not ancestor"
...
def test_pr159s_pr163_downstream_branch_allows_cumulative_validation(monkeypatch):
    monkeypatch.delenv("GITHUB_ACTIONS", raising=False)
    monkeypatch.delenv("GITHUB_EVENT_NAME", raising=False)
    monkeypatch.delenv("GITHUB_REF", raising=False)
    monkeypatch.delenv("GITHUB_REF_NAME", raising=False)
    ...
    def fake_git_stdout(repo_root, args):
        command = tuple(args)
        if command == ("branch", "--show-current"):
            return 0, PR163_DOWNSTREAM_BRANCH, ""
        if command == ("rev-parse", "--abbrev-ref", "HEAD"):
            return 0, PR163_DOWNSTREAM_BRANCH, ""
        if command[:2] == ("merge-base", "--is-ancestor"):
            return 1, "", "not ancestor"
...
def test_pr159s_detached_head_pr163_c_repair_pr_context_allowed(monkeypatch):
    ...
    assert failures == ()
    assert receipts == ("PR159S_REASON_CI_DETACHED_HEAD_RELAXATION_BRANCH_ONLY",)


def test_pr159s_detached_head_unrelated_repair_head_ref_remains_blocked(
    monkeypatch,
):
    failures, receipts = _detached_branch_outcome(
        monkeypatch,
    ...
    assert failures == ("PR159S_BLOCKED_WRONG_BRANCH:DETACHED_HEAD",)
    assert receipts == ()
