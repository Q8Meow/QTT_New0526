...
def test_repo_pr124_does_not_imply_roadmap_pr124_and_owner_authorized_connector_gate():
    pr124 = _entry("PR124_REPO_CANONICAL_SELF_ENTRY")
    roadmap_124 = _entry("ROADMAP_PR_124_PLANNED")

    assert pr124["repo_canonical_pr_label"] == "PR124"
    assert pr124["github_pr_number"] == 124
    assert pr124["github_pr_state"] == "MERGED"
    assert pr124["github_pr_mergedAt"] == "2026-05-19T09:57:48Z"
    assert (
        pr124["github_pr_mergeCommit_oid"]
        == "bc77112be515414837a46fba81abded63c956373"
    )
    assert pr124["github_pr_number"] != 124 or pr124["roadmap_pr_label"] is None
...
def test_pr124_github_audit_currentization_receipt_is_recorded_and_audit_only():
    pr124 = _entry("PR124_REPO_CANONICAL_SELF_ENTRY")
    receipt = json.loads(PR124_AUDIT_RECEIPT_PATH.read_text(encoding="utf-8"))

    assert receipt["repo_pr_label"] == "PR125"
    assert receipt["currentized_prior_repo_pr_label"] == "PR124"
    assert receipt["github_pr_number"] == 124
    assert receipt["github_pr_title"] == pr124["github_title"]
    assert receipt["github_pr_state"] == "MERGED"
    assert receipt["github_pr_mergedAt"] == "2026-05-19T09:57:48Z"
    assert (
        receipt["github_pr_mergeCommit_oid"]
        == "bc77112be515414837a46fba81abded63c956373"
    )
    assert (
        receipt["github_headRefName"]
        == "pr124-accepted-source-to-connector-semantic-binding-consumer-gate"
    )
    assert receipt["github_baseRefName"] == "main"
    assert receipt["same_number_inference_used"] is False
    assert pr124["roadmap_pr_label"] is None
    assert pr124["blueprint_pr_label"] is None


def test_repo_pr125_does_not_imply_roadmap_pr125_and_owner_authorized_pr107():
    ...
    assert roadmap_125["roadmap_title"] != roadmap_107["roadmap_title"]
    ...
    assert (
        ...
        == "SOURCE_REVALIDATION_SUPERSESSION_AND_MATERIALITY_SCHEDULER"
    )
    assert not any(
        entry["repo_canonical_pr_label"] == "PR125"
...
def test_pr127_github_audit_currentization_is_recorded_and_audit_only():
    pr127 = _entry("PR127_REPO_CANONICAL_SELF_ENTRY")
    receipt = json.loads(PR127_AUDIT_RECEIPT_PATH.read_text(encoding="utf-8"))

    assert pr127["repo_canonical_pr_label"] == "PR127"
    assert pr127["github_pr_number"] == 127
    assert pr127["github_title"] == "PR127 implement per-venue execution lifecycle model builder"
    assert pr127["github_pr_state"] == "MERGED"
    assert pr127["github_pr_mergedAt"] == "2026-05-19T23:11:19Z"
    assert pr127["github_pr_mergeCommit_oid"] == (
        "84848c0bbe2fb17ca02b76acd6cca45150ac59f9"
    )
    assert pr127["roadmap_pr_label"] is None
    assert pr127["blueprint_pr_label"] is None
    assert pr127["identity_relation_class"] == "REPO_CANONICAL_ONLY"
    assert pr127["same_number_mismatch_recorded"] is True

    assert receipt["repo_pr_label"] == "PR128"
    assert receipt["currentized_prior_repo_pr_label"] == "PR127"
    assert receipt["github_pr_number"] == 127
    assert receipt["github_pr_title"] == pr127["github_title"]
    assert receipt["github_pr_state"] == "MERGED"
    assert receipt["github_pr_mergedAt"] == "2026-05-19T23:11:19Z"
    ...
    assert receipt["github_baseRefName"] == "main"
    assert receipt["github_url"] == "https://github.com/Q8Meow/QTT_New0526/pull/127"
    assert receipt["same_number_inference_used"] is False


def test_repo_pr128_does_not_imply_roadmap_pr128_and_owner_authorized_pr110():
    receipt = json.loads(PR127_AUDIT_RECEIPT_PATH.read_text(encoding="utf-8"))

    assert not any(
        entry["repo_canonical_pr_label"] == "PR128"
        and entry["roadmap_pr_label"] == "PR #128"
        for entry in _entries()
    )
    assert receipt["repo_pr_label"] == "PR128"
    ...
    assert (
        receipt["owner_authorized_next_capability"]
        == "CROSS_VENUE_EXECUTION_NORMALIZATION_BINDING"
    )
    assert receipt["implementation_scope"] == (
        "ROADMAP_PR110_CROSS_VENUE_EXECUTION_NORMALIZATION_BINDING"
    )
    assert receipt["identity_doctrine_preserved"] is True
    assert receipt["controller_used_as_record_not_veto"] is True


def test_roadmap_and_controller_files_are_state_evidence_not_veto_for_pr128():
    roster = _roster()
    receipt = json.loads(PR127_AUDIT_RECEIPT_PATH.read_text(encoding="utf-8"))
...
def test_pr129_github_audit_currentization_is_recorded_and_audit_only():
    pr129 = _entry("PR129_REPO_CANONICAL_SELF_ENTRY")
    receipt = json.loads(PR129_AUDIT_RECEIPT_PATH.read_text(encoding="utf-8"))

    assert pr129["repo_canonical_pr_label"] == "PR129"
    assert pr129["github_pr_number"] == 129
    assert pr129["github_title"] == "PR129 implement runtime cash component field-map executor"
    assert pr129["github_pr_state"] == "MERGED"
    assert pr129["github_pr_mergedAt"] == "2026-05-20T03:24:12Z"
    assert pr129["github_pr_mergeCommit_oid"] == (
        "fad25e8fcc75b1ffc78e93705d36129ba9eab291"
    )
    assert pr129["github_headRefName"] == (
        "pr129-runtime-cash-component-field-map-executor"
    )
    assert pr129["github_baseRefName"] == "main"
    assert pr129["roadmap_pr_label"] is None
    assert pr129["blueprint_pr_label"] is None
    assert pr129["identity_relation_class"] == "REPO_CANONICAL_ONLY"
    assert pr129["same_number_mismatch_recorded"] is True

    assert receipt["repo_pr_label"] == "PR130"
    assert receipt["currentized_prior_repo_pr_label"] == "PR129"
    assert receipt["github_pr_number"] == 129
    assert receipt["github_pr_title"] == pr129["github_title"]
    assert receipt["github_pr_state"] == "MERGED"
    assert receipt["github_pr_mergedAt"] == "2026-05-20T03:24:12Z"
    assert receipt["github_pr_mergeCommit_oid"] == (
        "fad25e8fcc75b1ffc78e93705d36129ba9eab291"
    )
    assert receipt["github_url"] == "https://github.com/Q8Meow/QTT_New0526/pull/129"
    assert receipt["same_number_inference_used"] is False


def test_repo_pr130_does_not_imply_roadmap_pr130_and_owner_authorized_pr112():
    receipt = json.loads(PR129_AUDIT_RECEIPT_PATH.read_text(encoding="utf-8"))
    roadmap_112 = _entry("GITHUB_PR_112_IDENTITY_MISMATCH")

    assert roadmap_112["roadmap_title"] == (
        "Account, wallet, balance, and private-state read receipt gate"
    )
    assert not any(
        entry["repo_canonical_pr_label"] == "PR130"
        and entry["roadmap_pr_label"] == "PR #130"
        for entry in _entries()
    )
    assert receipt["repo_pr_label"] == "PR130"
    ...
    assert receipt["implementation_scope"] == (
        "ROADMAP_PR112_ACCOUNT_WALLET_BALANCE_AND_PRIVATE_STATE_READ_RECEIPT_GATE"
    )
    assert receipt["same_number_inference_used"] is False


def test_roadmap_and_controller_files_are_state_evidence_not_veto_for_pr130():
    pr129 = _entry("PR129_REPO_CANONICAL_SELF_ENTRY")
    receipt = json.loads(PR129_AUDIT_RECEIPT_PATH.read_text(encoding="utf-8"))

    assert _roster()["mismatch_summary"][
        "roadmap_controller_files_used_as_state_evidence_not_veto"
    ] is True
    assert pr129["controller_used_as_record_not_veto"] is True
    assert receipt["controller_used_as_record_not_veto"] is True
    assert receipt["identity_doctrine_preserved"] is True
    assert receipt["owner_authorized_roadmap_pr"] == "PR112"
