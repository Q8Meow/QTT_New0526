...
def test_inventory_has_pr168_recovery1_entries():
    entries = inventory.inventory_by_id()
    build_entry = entries["build_pr168_recovery1"]
    validate_entry = entries["validate_pr168_recovery1"]

    for entry in (build_entry, validate_entry):
        assert entry.owner_pr_or_feature == "PR168_RECOVERY1"
        assert "docs/master_plan/generated/PR168_RECOVERY1_*.report.json" in entry.output_globs
        assert "docs/master_plan/generated/recovery1/**" in entry.output_globs
        assert "tests/pr168_recovery1/**" in entry.required_when_files_match

    assert "tools/build_pr168_recovery1.py" in build_entry.tool_globs
    assert "tools/validate_pr168_recovery1.py" in validate_entry.tool_globs
