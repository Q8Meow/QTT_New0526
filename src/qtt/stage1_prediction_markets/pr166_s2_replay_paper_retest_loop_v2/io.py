"""JSON, path, branch, and shard helpers for PR166-S2."""
...
def records_from_report_payload(repo_root: Path, payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for shard_ref in payload.get("shard_files") or []:
        ...
        for row in records_from_payload(shard_payload):
            annotated = dict(row)
            annotated.setdefault("input_shard_refs", [normalize_repo_ref(shard_ref)])
            annotated["_source_shard_ref"] = normalize_repo_ref(shard_ref)
