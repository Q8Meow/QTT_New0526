"""I/O helpers for PR165-D3 artifacts."""
...
def ensure_branch(repo_root: Path) -> None:
    ...
    if ci_branch in {c.EXPECTED_BRANCH, c.BASE_BRANCH, ""}: return
    raise RuntimeError(f"{c.PR_ID} builder must run on {c.EXPECTED_BRANCH} or {c.BASE_BRANCH}; current branch context is {branch or ci_branch or 'UNKNOWN'}")
def _current_branch(repo_root: Path) -> str:
    p = subprocess.run(["git", "branch", "--show-current"], cwd=repo_root, check=False, capture_output=True, text=True); return p.stdout.strip() if p.returncode == 0 else ""
