"""Replay run result registry builder for PR166-S."""
...
def build_replay_run_result_rows(
    ...
    for index, episode in enumerate(replay_episode_rows, start=1):
        batch_id = str(episode["source_selected_batch_id"])
        attrs = attrs_by_batch.get(batch_id, [])
        row_id = f"PR166_S_REPLAY_RUN_RESULT::{index:06d}"
        rows.append(
            {
                "replay_run_result_id": row_id,
                "run_id": episode["run_id"],
                "source_selected_batch_id": batch_id,
                "run_mode": "REPLAY",
                "run_status": "REPLAY_EXECUTED" if attrs else "REPAIR_REQUIRED_BEFORE_EXECUTION",
                ...
                "average_net_edge_after_costs": _avg(attrs, "net_return_proxy"),
                ...
                **row_contract(
                    row_id=row_id,
                    source_artifact_ref="PR166_S_ReplayEpisodeRegistry.report.json",
                    source_row_ref=episode["replay_episode_id"],
                    computed_by_module="replay_run_engine",
                    owning_agent="replay_agent",
                    consuming_agent="commander_agent",
                    downstream_action_type="replay run result handoff input",
