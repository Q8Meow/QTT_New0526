"""Windows-safe artifact path validation for PR168-RP5D."""
...
UNICODE_PUNCTUATION_RE = re.compile(r"[^\x00-\x7f]")
...
def windows_absolute_length(filename: str) -> int:
    rel = repo_relative_path(filename).replace("/", "\\")
    return len(WINDOWS_REPO_ROOT_ASSUMPTION + rel)
...
def filename_is_safe(filename: str) -> bool:
    ...
    return (
        ...
        and not UNICODE_PUNCTUATION_RE.search(name)
    )
