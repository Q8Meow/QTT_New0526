"""Deterministic report sharding for PR161E."""
...
from .compact_records import compact_records_for_report, hoist_compact_record_defaults
    ...
    *,
    ...
    for filename in c.REPORT_FILENAMES:
        ...
        for index, chunk in enumerate(chunks, start=1):
            ...
            compact_record_defaults, compact_chunk = hoist_compact_record_defaults(compact_chunk)
            shard_payload = dict(payload)
            shard_payload["records"] = compact_chunk
            shard_payload["compact_records_flag"] = compact_chunk != chunk
            shard_payload["compact_record_version"] = (
                shared_dictionary.get("compact_record_version")
                if shard_payload["compact_records_flag"]
                else None
            )
            shard_payload["compact_record_defaults"] = compact_record_defaults
            shard_payload["shared_dictionary_ref"] = (
                c.SHARED_DICTIONARY_REPORT_PATH.as_posix()
                if shard_payload["compact_records_flag"]
                else None
            )
        ...
        preview_records = records[: c.REPORT_SHARD_PREVIEW_RECORD_LIMIT]
        payload["records"] = []
        payload["preview_records"] = preview_records
        payload["preview_record_count"] = len(preview_records)
        ...
        payload["full_records_canonical_location"] = "PR161E_COMPACT_SHARDS_PLUS_SHARED_DICTIONARY"
        payload["compact_records_flag"] = True
        payload["shared_dictionary_ref"] = c.SHARED_DICTIONARY_REPORT_PATH.as_posix()
        main_payloads[filename] = payload
        manifest_records.append(
            {
                "record_id": f"PR161E-SHARD-MANIFEST-{Path(filename).stem}",
                ...
                "canonical_records_location": "PR161E_COMPACT_SHARDS_PLUS_SHARED_DICTIONARY",
            }
        )
