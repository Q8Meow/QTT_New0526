"""Write deterministic PR162 JSON schemas from central policy constants."""
...
def _generic_report_schema(report_filename: str, schema_filename: str) -> dict[str, Any]:
    ...
    for field, values in c.SCHEMA_ENUM_FIELDS.items():
        record_properties[field] = {
            "enum": list(values),
            "x-pr162-enum-source": f"{c.PACKAGE_IMPORT}.constants:{field}",
        }
    ...
    if report_filename == "PR162_FinalSummary.report.json":
        report_properties["pr152_currentization_result"] = {
            "enum": list(c.PR152_CURRENTIZATION_RESULTS),
            "x-pr162-enum-source": f"{c.PACKAGE_IMPORT}.constants:PR152_CURRENTIZATION_RESULTS",
        }
    return {
        ...
        "additionalProperties": True,
        "x-pr162-schema-enum-source": f"{c.PACKAGE_IMPORT}.constants",
    }
