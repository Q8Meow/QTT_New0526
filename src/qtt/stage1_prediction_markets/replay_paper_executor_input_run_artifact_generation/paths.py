"""Path normalization and traversal protections for PR161F shards."""
...
def normalize_shard_ref(repo_root: Path, shard_ref: Any) -> str:
    ...
    if normalized.startswith("//") or normalized.startswith("\\\\"):
        raise ValueError(f"PR161F shard path must not be UNC: {raw_path}")
    if posix_path.is_absolute() or windows_path.is_absolute() or windows_path.drive:
        raise ValueError(f"PR161F shard path must be relative: {raw_path}")
    if any(part == ".." for part in posix_path.parts):
        raise ValueError(f"PR161F shard path must not contain '..': {raw_path}")
    candidate = repo_root.joinpath(*posix_path.parts)
    resolved_root = repo_root.resolve(strict=False)
    try:
        candidate.resolve(strict=False).relative_to(resolved_root)
    except ValueError as exc:
        raise ValueError(f"PR161F shard path escapes repo root: {raw_path}") from exc
