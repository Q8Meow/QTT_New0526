"""Windows-safe path checks for RP5D-R1 generated overlays."""
...
def filename_is_safe(filename: str) -> bool:
    ...
    return name == filename and len(name) <= MAX_GENERATED_FILENAME_CHARS and not UNSAFE_FILENAME_RE.search(name) and not NON_ASCII_RE.search(name)
