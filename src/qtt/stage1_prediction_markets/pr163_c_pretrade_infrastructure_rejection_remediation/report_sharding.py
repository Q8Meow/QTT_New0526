"""Report sharding helpers for PR163-C row-level registries."""
...
def build_sharded_payloads(
    ...
    for index, chunk in enumerate(chunks, start=1):
        ...
        shard_payload.update(
            {
                ...
                "part_ref": f"PR163C_PART::{index:04d}",
                "part_index": index,
        ...
        shard_size = encoded_json_size(shard_payload, compact=True)
        shard_manifest_refs.append(
            {
                ...
                "shard_index": index,
                "row_count": len(chunk),
                "estimated_shard_size_bytes": shard_size,
                "below_25_mib_limit": shard_size <= SHARD_LIMIT_BYTES,
            }
        )
...
def aggregate_counts(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if row.get("qku_ids"):
            qkus.update(str(qku) for qku in row.get("qku_ids") or [])
