...
def _validate_atomicrows_authority_state(
    ...
    *,
    ...
    if bundle_present:
        failures.append(
            "canonical AtomicRows bundle must remain absent during source-evidence "
            f"gate confirmation: {CANONICAL_BUNDLE_RELATIVE_PATH}"
        )
    if sha_present:
        failures.append(
            "canonical AtomicRows bundle hash must remain absent during source-evidence "
            f"gate confirmation: {CANONICAL_BUNDLE_SHA_RELATIVE_PATH}"
        )
    return failures
