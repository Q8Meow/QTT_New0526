...
def _validate_atomicrows_state(
    ...
    *,
    ...
    if bundle_present:
        failures.append(
            "canonical AtomicRows bundle must remain absent during venue-neutral "
            f"adapter validation: {CANONICAL_BUNDLE_RELATIVE_PATH}"
        )
    if sha_present:
        failures.append(
            "canonical AtomicRows bundle hash must remain absent during venue-neutral "
            f"adapter validation: {CANONICAL_BUNDLE_SHA_RELATIVE_PATH}"
        )
    return failures
