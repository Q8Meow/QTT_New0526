...
def _validate_false_map(
    ...
    failures = _require_fields(value, expected_fields, label)
...
def _validate_scope_flags(fixture: dict[str, Any]) -> list[str]:
    ...
    if scope_flags is None:
        return failures
    failures.extend(_require_fields(scope_flags, TRUE_SCOPE_FLAGS | FALSE_SCOPE_FLAGS, "scope_flags"))
...
def _validate_schema(schema: dict[str, Any]) -> list[str]:
    failures = _require_fields(schema, {"$schema", "$id", "type", "properties", "required", "$defs"}, "schema")
    ...
    if isinstance(properties, dict):
        failures.extend(_require_fields(properties, ROOT_FIELDS, "schema.properties"))
    else:
        failures.append("schema.properties must be an object")
    if isinstance(required, list):
        missing_required = sorted(ROOT_FIELDS - set(required))
        if missing_required:
    ...
    if isinstance(defs, dict):
        failures.extend(
            _require_fields(
                defs,
                {"scope_flags", "order_intent_placeholder", "execution_router_gate_placeholder"},
                "schema.$defs",
            )
        )
    else:
...
def _validate_order_intent_placeholder(surface: dict[str, Any], label: str) -> list[str]:
    ...
    failures = _require_fields(surface, required, label)
    ...
    if semantics is not None:
        failures.extend(_require_fields(semantics, SEMANTIC_FIELDS, f"{label}.source_required_order_intent_fields"))
    return failures
...
def _validate_global_fail_closed_values(fixture: dict[str, Any]) -> list[str]:
    ...
    for fragment in sorted(FORBIDDEN_TEXT_FRAGMENTS):
        if fragment in raw_text:
            failures.append(f"fixture contains forbidden live/source/private fragment: {fragment}")
    return failures


def validate_order_intent_execution_router_fixture(fixture: dict[str, Any]) -> list[str]:
    failures = _require_fields(fixture, ROOT_FIELDS, "order intent fixture")
    expected = {
    ...
    if fixture.get("deterministic_output") is not True:
        failures.append("order intent fixture.deterministic_output must be true")
