...
PYTEST_BASETEMP_LEGACY_FIXED_DIR_NAME = "run_validation_gates_pytest"
PYTEST_BASETEMP_PARENT = f"qtt_{PYTEST_BASETEMP_LEGACY_FIXED_DIR_NAME}"
...
PR168_RP5F_DETERMINISTIC_SCRIPT_NAMES = frozenset(
    {
        "build_pr168_rp5f_dynamic_targets.py",
...
PR169_READINESS1_DETERMINISTIC_SCRIPT_NAMES = frozenset(
    {
...
PR169_PRETRADE1_DETERMINISTIC_SCRIPT_NAMES = frozenset(
    {
        "build_pr169_pretrade1.py",
...
PYTEST_SHARD_COMMANDS: dict[str, tuple[PytestShardCommand, ...]] = {
    ...
    "pytest-shard-2": (
        ...
        PytestShardCommand(
            paths=("tests/pr168_rank",),
            reason="PR168-RANK evidence-backed ranking, simulation, and registry seed tests",
            runtime_budget_seconds=PYTEST_SUBPROCESS_GROUP_TARGET_SECONDS,
        ...
        PytestShardCommand(
            paths=("tests/pr168_data1a",),
            reason="PR168-DATA1A focused DATA1 audit and GFP2R readiness tests",
            runtime_budget_seconds=PYTEST_SUBPROCESS_GROUP_TARGET_SECONDS,
            historical_runtime_seconds=20.0,
        ),
        PytestShardCommand(
            paths=("tests/pr168_gfp2r",),
            reason="PR168-GFP2R DATA1A-gated candidate formula recompute tests",
            runtime_budget_seconds=PYTEST_SUBPROCESS_GROUP_TARGET_SECONDS,
            historical_runtime_seconds=20.0,
        ),
        PytestShardCommand(
            paths=("tests/pr168_rp2",),
            reason="PR168-RP2-MAP2 GFP2R-gated replay/paper recompute tests",
            runtime_budget_seconds=PYTEST_SUBPROCESS_GROUP_TARGET_SECONDS,
            historical_runtime_seconds=20.0,
        ),
        PytestShardCommand(
            paths=("tests/pr168_map3",),
            reason="PR168-MAP3 canonical identity intake and formula materialization tests",
            runtime_budget_seconds=PYTEST_SUBPROCESS_GROUP_TARGET_SECONDS,
            historical_runtime_seconds=20.0,
        ),
        PytestShardCommand(
        ...
        PytestShardCommand(
            paths=("tests/pr168_rank3",),
            reason="PR168-RANK3 RP3 evidence-backed stack ranking tests",
            runtime_budget_seconds=PYTEST_SUBPROCESS_GROUP_TARGET_SECONDS,
            historical_runtime_seconds=20.0,
        ),
        PytestShardCommand(
            paths=("tests/pr168_rp5a",),
            reason="PR168-RP5A legacy semantic audit and cleanup-readiness tests",
            runtime_budget_seconds=PYTEST_SUBPROCESS_GROUP_TARGET_SECONDS,
            historical_runtime_seconds=20.0,
        ),
        PytestShardCommand(
        ...
        PytestShardCommand(
            paths=("tests/pr168_rp5d",),
            reason="PR168-RP5D replay/paper executability and adapter queue tests",
            runtime_budget_seconds=PYTEST_SUBPROCESS_GROUP_TARGET_SECONDS,
            historical_runtime_seconds=20.0,
        ),
        PytestShardCommand(
        ...
        PytestShardCommand(
            paths=("tests/pr169_readiness1",),
            reason="PR169-READINESS1 centralized readiness contract tests",
            runtime_budget_seconds=PYTEST_SUBPROCESS_GROUP_TARGET_SECONDS,
            historical_runtime_seconds=5.0,
        ),
        PytestShardCommand(
            paths=("tests/pr169_pretrade1",),
            reason="PR169-PRETRADE1 pretrade reality-model and no-submit packet tests",
            runtime_budget_seconds=PYTEST_SUBPROCESS_GROUP_TARGET_SECONDS,
            historical_runtime_seconds=5.0,
        ),
        PytestShardCommand(
...
def build_validation_commands(
    ...
    commands = [
        ...
        [
            sys.executable,
            _path("tools", "validate_pr168_data1a_focused_audit.py"),
        ],
        [
            sys.executable,
            _path("tools", "validate_pr168_gfp2r_data1a_gated_candidate_recompute.py"),
        ],
        [
            sys.executable,
            _path("tools", "build_pr168_rp2_map2.py"),
            "--offline",
        ],
        [
            sys.executable,
            _path("tools", "validate_pr168_rp2_map2.py"),
        ],
        [
            sys.executable,
            _path("tools", "build_pr168_map3.py"),
            "--offline",
        ],
        [
            sys.executable,
        ...
        [
            sys.executable,
            _path("tools", "build_pr168_rank3.py"),
            "--offline",
        ],
        [
            sys.executable,
        ...
        [
            sys.executable,
            _path("tools", "build_pr168_rp5a_legacy_semantic_audit.py"),
            "--offline",
        ],
        [
            sys.executable,
        ...
        [
            sys.executable,
            _path("tools", "build_pr168_rp5d_replay_paper_executability_tiers.py"),
            "--offline",
        ],
        [
            sys.executable,
            _path("tools", "validate_pr168_rp5d_replay_paper_executability_tiers.py"),
        ],
        [
            sys.executable,
            _path("tools", "build_pr168_rp5e_stack_gen.py"),
            "--offline",
            "--fixture",
            "sample",
            "--max-stacks",
        ...
        [
            sys.executable,
            _path("tools", "build_pr168_rp5f_dynamic_targets.py"),
            "--offline",
            "--fixture",
            "sample",
            "--max-targets",
        ...
        [
            sys.executable,
            _path("tools", "validate_pr169_readiness1.py"),
            "--repo-root",
            ".",
            "--artifact-dir",
            "docs/master_plan/generated/pr169_readiness1",
        ...
        [
            sys.executable,
            _path("tools", "build_pr169_pretrade1.py"),
            "--repo-root",
            ".",
            "--out-dir",
        ...
        [
            sys.executable,
            _path("tools", "validate_pr169_pretrade1.py"),
            "--repo-root",
            ".",
            "--artifact-dir",
            "docs/master_plan/generated/pr169_pretrade1",
...
def _rp5f_local_branch_scope_active(
    *,
    repo_root: pathlib.Path,
    phase: str,
    validation_mode: str,
    ...
    if validation_mode != "auto" or changed_files or force_full or manual_mode:
        return False
    return _current_git_branch(repo_root) == PR168_RP5F_BRANCH


def _filter_commands_for_rp5f_local_branch_scope(
    commands: Sequence[Sequence[str]],
) -> list[list[str]]:
    kept = [
        list(command)
        for command in commands
        if _command_script_name(command) in PR168_RP5F_DETERMINISTIC_SCRIPT_NAMES
    ]
    skipped = [
        _command_script_name(command)
        for command in commands
        if _command_script_name(command) not in PR168_RP5F_DETERMINISTIC_SCRIPT_NAMES
    ]
    if skipped:
        print(
            "QTT_RP5F_LOCAL_BRANCH_SCOPE_SKIPPED "
            f"phase={DETERMINISTIC_VALIDATORS_PHASE} "
            f"scripts={','.join(sorted(name for name in skipped if name))}",
            flush=True,
        )
...
def _write_rp5f_local_branch_scope_report(
    router_report_path: pathlib.Path | None,
    *,
    repo_root: pathlib.Path,
    kept_commands: Sequence[Sequence[str]],
    ...
    router_report_path.write_text(
        json.dumps(
            {
                "routing_policy": "RP5F_LOCAL_BRANCH_AFFECTED_SCOPE",
                "branch": PR168_RP5F_BRANCH,
                "phase": DETERMINISTIC_VALIDATORS_PHASE,
                "full_validation_required": False,
                "required_scripts": [
                    _command_script_name(command) for command in kept_commands
                ...
                "reason": (
                    "Local deterministic-validators on PR168-RP5F validates the "
                    "branch-owned dynamic target and order-variable grid "
                    "build/validator pair. RP5C/VS1/RP5D/RP5E/RP5D-R1 remain "
                    "read-only upstream inputs and are not rebuilt on this branch."
                ),
            },
            indent=2,
            sort_keys=True,
...
def _pr169_readiness1_local_branch_scope_active(
    *,
    repo_root: pathlib.Path,
    phase: str,
    validation_mode: str,
    ...
    if validation_mode != "auto" or changed_files or force_full or manual_mode:
        return False
    return _current_git_branch(repo_root) == PR169_READINESS1_BRANCH


def _filter_commands_for_pr169_readiness1_local_branch_scope(
    commands: Sequence[Sequence[str]],
) -> list[list[str]]:
    kept = [
        list(command)
        for command in commands
        if _command_script_name(command) in PR169_READINESS1_DETERMINISTIC_SCRIPT_NAMES
    ]
    skipped = [
        _command_script_name(command)
        for command in commands
        if _command_script_name(command) not in PR169_READINESS1_DETERMINISTIC_SCRIPT_NAMES
    ]
    if skipped:
        print(
            "QTT_PR169_READINESS1_LOCAL_BRANCH_SCOPE_SKIPPED "
            f"phase={DETERMINISTIC_VALIDATORS_PHASE} "
            f"scripts={','.join(sorted(name for name in skipped if name))}",
            flush=True,
        )
...
def _write_pr169_readiness1_local_branch_scope_report(
    router_report_path: pathlib.Path | None,
    *,
    repo_root: pathlib.Path,
    kept_commands: Sequence[Sequence[str]],
    ...
    router_report_path.write_text(
        json.dumps(
            {
                "routing_policy": "PR169_READINESS1_LOCAL_BRANCH_AFFECTED_SCOPE",
                "branch": PR169_READINESS1_BRANCH,
                "phase": DETERMINISTIC_VALIDATORS_PHASE,
                "full_validation_required": False,
                "required_scripts": [
                    _command_script_name(command) for command in kept_commands
                ...
                "reason": (
                    "Local deterministic-validators on PR169-READINESS1 validates "
                    "the branch-owned readiness builder/validator pair. Upstream "
                    "RP5G/RANK4/QOPT1/VS2/MEM1 and PR169 owner UI artifacts remain "
                    "read-only inputs."
                ),
            },
            indent=2,
...
def _pr169_pretrade1_local_branch_scope_active(
    *,
    repo_root: pathlib.Path,
    phase: str,
    validation_mode: str,
    ...
    if validation_mode != "auto" or changed_files or force_full or manual_mode:
        return False
    return _current_git_branch(repo_root) == PR169_PRETRADE1_BRANCH


def _filter_commands_for_pr169_pretrade1_local_branch_scope(
    commands: Sequence[Sequence[str]],
) -> list[list[str]]:
    kept = [
        list(command)
        for command in commands
        if _command_script_name(command) in PR169_PRETRADE1_DETERMINISTIC_SCRIPT_NAMES
    ]
    skipped = [
        _command_script_name(command)
        for command in commands
        if _command_script_name(command) not in PR169_PRETRADE1_DETERMINISTIC_SCRIPT_NAMES
    ]
    if skipped:
        print(
            "QTT_PR169_PRETRADE1_LOCAL_BRANCH_SCOPE_SKIPPED "
            f"phase={DETERMINISTIC_VALIDATORS_PHASE} "
            f"scripts={','.join(sorted(name for name in skipped if name))}",
            flush=True,
        )
...
def _write_pr169_pretrade1_local_branch_scope_report(
    router_report_path: pathlib.Path | None,
    *,
    repo_root: pathlib.Path,
    kept_commands: Sequence[Sequence[str]],
    ...
    router_report_path.write_text(
        json.dumps(
            {
                "routing_policy": "PR169_PRETRADE1_LOCAL_BRANCH_AFFECTED_SCOPE",
                "branch": PR169_PRETRADE1_BRANCH,
                "phase": DETERMINISTIC_VALIDATORS_PHASE,
                "full_validation_required": False,
                "required_scripts": [
                    _command_script_name(command) for command in kept_commands
                ...
                "reason": (
                    "Local deterministic-validators on PR169-PRETRADE1 validates "
                    "the branch-owned pretrade builder/validator pair. READINESS1 "
                    "and MEM1 remain read-only upstream inputs."
                ),
            },
            indent=2,
            sort_keys=True,
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="qtt_validation_gates_") as temp_dir:
            ...
            with tempfile.TemporaryDirectory(
                ...
                if _rp5d_r1_local_branch_scope_active(
                ...
                elif _rp5f_local_branch_scope_active(
                    repo_root=repo_root,
                    phase=args.phase,
                    validation_mode=args.validation_mode,
                    changed_files=args.changed_file,
                ...
                elif _pr169_readiness1_local_branch_scope_active(
                    repo_root=repo_root,
                    phase=args.phase,
                    validation_mode=args.validation_mode,
                    changed_files=args.changed_file,
                    ...
                    print(
                        "QTT_PR169_READINESS1_LOCAL_BRANCH_SCOPE_MODE "
                        f"phase={args.phase} full_validation_required=False",
                        flush=True,
                    )
                elif _pr169_pretrade1_local_branch_scope_active(
                    repo_root=repo_root,
                    phase=args.phase,
                    validation_mode=args.validation_mode,
                    changed_files=args.changed_file,
                    ...
                    print(
                        "QTT_PR169_PRETRADE1_LOCAL_BRANCH_SCOPE_MODE "
                        f"phase={args.phase} full_validation_required=False",
                        flush=True,
                    )
